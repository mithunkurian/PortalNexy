...
async function run(){
 ...
 try {
  ...
  assert.equal(await home.locator('#chart-home-wip').count(),1);
  assert.match(await home.innerText(),/ETF rotation/);
  assert.match(await home.innerText(),/BTC \/ ETH momentum/);
  await home.getByRole('button',{name:/Waiting orders/}).click();
  await home.getByRole('button',{name:'View',exact:true}).first().click();
